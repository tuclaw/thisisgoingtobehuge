#!/usr/bin/env node
/** Thu Sep 24 2026 MID — contestant fills + living marks. Snapshot s1e08-thu-mid. */
...
const hostPath = join(root, "recs", "2026-09-24-mid", "season1.after-mid.json");
const booksPayload = JSON.parse(
  readFileSync(join(root, "recs", "2026-09-24-mid", "BOOKS.after.json"), "utf8")
);
const remake = JSON.parse(readFileSync(join(root, "recs", "2026-09-24-mid", "REMAKE.json"), "utf8"));
const fills = JSON.parse(readFileSync(join(root, "recs", "2026-09-24-mid", "FILLS.json"), "utf8"));
...
const thuOpen = (season.events || []).find((e) => e && e.id === "s1e08-thu-open");
if (!thuOpen) {
  console.error("s1e08-thu-open mark missing");
  process.exit(1);
}

if ((season.events || []).some((e) => e && e.id === "s1e08-thu-mid")) {
  console.error("s1e08-thu-mid already present — abort");
  process.exit(1);
}
...
for (const [ticker, last] of Object.entries(LAST_QUOTES)) {
  ...
  const prior = thuOpen.marks?.[ticker] ?? q.priorClose;
  season.quotes[ticker] = {
    ...
    priorCloseSource: "robinhood-last (Thu Sep 24 open · s1e08-thu-open)",
    interpolated: false
  };
}
...
for (const f of fills.contestantFills || []) {
  ...
  midFills.push({
    ...
    orderId: f.orderId,
    at: f.filledAt,
    note: f.note || `Thu Sep 24 mid ${f.side} ${f.ticker}`
  });
}
...
season.events.push({
  ...
  label:
    "Thu Sep 24 2026 MID · robinhood last after fills (~9:57 AM PT). Snapshot s1e08-thu-mid. Contestant fills. MERGED · four living.",
  ...
  fillsSinceLastHour: [],
  marks: LAST_QUOTES,
  upgradesSnapshotId: "s1e08-thu-open"
});
...
season.markLabel =
  "Thu Sep 24 mid remake · Robinhood last · snapshot s1e08-thu-mid. hunt-brain 4/4. Leader Claude Opus 5 -0.3430%. Worst Claude Sonnet 5 -3.4856%.";
...
season.statusLabel =
  "Episode 8 · Thu mid · four living · MERGED · comics paused";
season.lastSource = QUOTE_SOURCE;
season.lastSession = LAST_SESSION;
season.notes =
  "Season live. S1E08 live Wed Sep 23 – Fri Sep 25. MERGED. Four living. Thu mid remake after hunt-brain fills. Given $361.93. Pot $376.1318. Claude Opus 5 leads −0.34% and wears immunity. Claude Sonnet 5 worst −3.49%. hunt-brain 4/4. Comics paused. Audience only. Tribal Fri Sep 25 2:00 PM PT.";
...
console.log("Applied s1e08-thu-mid · islandPotUsd", season.islandPotUsd, "· immunity Claude Opus 5 -0.34%");
