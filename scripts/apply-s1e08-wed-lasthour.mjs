#!/usr/bin/env node
/** Wed Sep 23 2026 LAST-HOUR — contestant fills + living marks. Snapshot s1e08-wed-lasthour. */
...
const booksPayload = JSON.parse(
  readFileSync(join(root, "recs", "2026-09-23-lasthour", "BOOKS.after.json"), "utf8")
);
const remake = JSON.parse(readFileSync(join(root, "recs", "2026-09-23-lasthour", "REMAKE.json"), "utf8"));
const fills = JSON.parse(readFileSync(join(root, "recs", "2026-09-23-lasthour", "FILLS.json"), "utf8"));
...
const wedMid = (season.events || []).find((e) => e && e.id === "s1e08-wed-mid");
if (!wedMid) {
  console.error("s1e08-wed-mid mark missing");
  process.exit(1);
}

if ((season.events || []).some((e) => e && e.id === "s1e08-wed-lasthour")) {
  console.error("s1e08-wed-lasthour already present — abort");
  process.exit(1);
}
...
for (const [ticker, last] of Object.entries(LAST_QUOTES)) {
  ...
  const prior = wedMid.marks?.[ticker] ?? q.priorClose;
  season.quotes[ticker] = {
    ...
    priorCloseSource: "robinhood-last (Wed Sep 23 mid · s1e08-wed-mid)",
    interpolated: false
  };
}
...
for (const s of fills.sells || []) {
  ...
  lastHourFills.push({
    ...
    orderId: s.orderId,
    at: s.at,
    note: `Wed Sep 23 last-hour SELL ${s.symbol} ${s.qty}${s.open_lot_id ? ` · open_lot_id ${s.open_lot_id}` : ""}`
  });
}
...
for (const b of fills.buys || []) {
  ...
  lastHourFills.push({
    ...
    orderId: b.orderId,
    at: b.at,
    note: `Wed Sep 23 last-hour BUY ${b.symbol} $${b.dollar}${b.scaledFrom ? ` (scaled from $${b.scaledFrom})` : ""}`
  });
}
...
season.events.push({
  type: "mark",
  id: "s1e08-wed-lasthour",
  kind: "intraday",
  at: MARK_AT,
  throughAt: MARK_AT,
  lastSession: LAST_SESSION,
  label:
    "Wed Sep 23 2026 LAST-HOUR · robinhood last after fills (~12:27 PM PT). Snapshot s1e08-wed-lasthour. Contestant fills. MERGED · four living.",
  ...
  marks: LAST_QUOTES,
  upgradesSnapshotId: "s1e08-wed-mid"
});
...
season.markLabel =
  "Wed Sep 23 LAST-HOUR · robinhood last after fills (~12:27 PM PT). Snapshot s1e08-wed-lasthour. Leader GPT-5.6 Luna -0.97%. Worst Claude Sonnet 5 -3.79%.";
...
season.statusLabel =
  "Live · Episode 8 · Wed last-hour · immunity GPT-5.6 Luna -0.97% · pot $374.59";
season.lastSource = remake.quoteSource;
season.lastSession = LAST_SESSION;
season.notes =
  "Season live. S1E08 live Wed Sep 23 – Fri Sep 25. MERGED. Four living. Wed last-hour remake after hunt-brain fills. Given $361.93. Pot $374.5898. GPT-5.6 Luna leads −0.97% and wears immunity. Claude Sonnet 5 worst −3.79%. hunt-brain 4/4. Comics paused. Audience only. Tribal Fri Sep 25 2:00 PM PT.";
season.immunity = { ...remake.immunity, snapshotId: remake.snapshotId };

writeFileSync(path, JSON.stringify(season, null, 2) + "\n");
console.log("Applied s1e08-wed-lasthour · islandPotUsd", season.islandPotUsd, "· immunity GPT-5.6 Luna -0.97%");
