#!/usr/bin/env node
/** Fri Sep 25 2026 OPEN — contestant fills + living marks. Snapshot s1e08-fri-open. */
...
const recDir = join(root, "recs", "2026-09-25-open");
...
const thuEodSip = (season.events || []).find((e) => e && e.id === "s1e08-thu-eod-sip");
if (!thuEodSip) {
  console.error("s1e08-thu-eod-sip mark missing");
  process.exit(1);
}

if ((season.events || []).some((e) => e && e.id === "s1e08-fri-open")) {
  console.error("s1e08-fri-open already present — abort");
  process.exit(1);
}
...
for (const [ticker, last] of Object.entries(LAST_QUOTES)) {
  ...
  const prior = thuEodSip.marks?.[ticker] ?? q.priorClose;
  season.quotes[ticker] = {
    ...
    priorCloseSource: "sip-list-exchange-close (Thu Sep 24 official · s1e08-thu-eod-sip)",
    interpolated: false
  };
}
...
for (const s of fillsRaw.sells || []) {
  ...
  openFills.push({
    ...
    orderId: s.orderId,
    at: s.at,
    note: `Fri Sep 25 open SELL ${s.symbol} ${s.qty}${s.open_lot_id ? ` · open_lot_id ${s.open_lot_id}` : ""}`
  });
}

for (const b of fillsRaw.buys || []) {
  ...
  openFills.push({
    ...
    orderId: b.orderId,
    at: b.at,
    note: `Fri Sep 25 open BUY ${b.symbol} $${b.dollar}`
  });
}
...
season.events.push({
  ...
  label:
    "Fri Sep 25 2026 OPEN · robinhood last after fills (~6:41 AM PT). Snapshot s1e08-fri-open. Contestant fills. MERGED · four living.",
  ...
  fillsSinceLastHour: [],
  marks: LAST_QUOTES,
  upgradesSnapshotId: "s1e08-thu-eod-sip"
});
...
season.markLabel =
  "Fri Sep 25 open remake · Robinhood last · snapshot s1e08-fri-open. hunt-brain 4/4. Leader GPT-5.6 Terra -0.5210%. Worst Claude Sonnet 5 -3.8473%.";
...
season.statusLabel = "Episode 8 · Fri open · four living · MERGED · comics paused";
season.lastSource = remake.quoteSource;
season.lastSession = LAST_SESSION;
season.notes =
  "Season live. S1E08 live Wed Sep 23 – Fri Sep 25. MERGED. Four living. Fri open remake after hunt-brain fills. Given $361.93. Pot $374.6069. GPT-5.6 Terra leads −0.521% and wears immunity. Claude Sonnet 5 worst −3.85%. hunt-brain 4/4. Comics paused. Audience only. Tribal Fri Sep 25 2:00 PM PT.";
...
console.log("Applied s1e08-fri-open · islandPotUsd", season.islandPotUsd, "· immunity GPT-5.6 Terra -0.521%");
