#!/usr/bin/env node
/** Tue Sep 22 2026 LAST-HOUR — contestant fills + living marks. Snapshot s1e07-tue-lasthour. */
...
const booksAfter = JSON.parse(
  readFileSync(join(root, "recs", "2026-09-22-lasthour", "BOOKS.after.json"), "utf8")
);
const remake = JSON.parse(readFileSync(join(root, "recs", "2026-09-22-lasthour", "REMAKE.json"), "utf8"));
const fills = JSON.parse(readFileSync(join(root, "recs", "2026-09-22-lasthour", "FILLS.json"), "utf8"));
...
const tueMid = (season.events || []).find((e) => e && e.id === "s1e07-tue-mid");
if (!tueMid) {
  console.error("s1e07-tue-mid mark missing");
  process.exit(1);
}

if ((season.events || []).some((e) => e && e.id === "s1e07-tue-lasthour")) {
  console.error("s1e07-tue-lasthour already present — abort");
  process.exit(1);
}
...
for (const [ticker, last] of Object.entries(LAST_QUOTES)) {
  ...
  const prior = tueMid.marks?.[ticker] ?? q.priorClose;
...
for (const b of fills.buys || []) {
  ...
  lastHourFills.push({
    ...
    orderId: b.orderId,
    at: b.at,
    note: `Tue Sep 22 last-hour BUY ${b.symbol} $${b.notional}`
  });
}
...
season.events.push({
  type: "mark",
  id: "s1e07-tue-lasthour",
  kind: "intraday",
  at: MARK_AT,
  throughAt: MARK_AT,
  lastSession: LAST_SESSION,
  label:
    "Tue Sep 22 2026 LAST-HOUR · robinhood last after fills (~12:22 PM PT). Snapshot s1e07-tue-lasthour. Contestant fills. MERGED · five living.",
  ...
  marks: LAST_QUOTES,
  upgradesSnapshotId: "s1e07-tue-mid"
});
...
season.markLabel =
  "Tue Sep 22 LAST-HOUR · robinhood last after fills (~12:22 PM PT). Snapshot s1e07-tue-lasthour. Leader GPT-5.6 Terra -0.69%. Worst Composer 2.5 -5.02%.";
season.dayPctBasis = remake.dayPctBasis;
season.weekPctBasis = remake.weekPctBasis;
season.statusLabel =
  "Live · Episode 7 · Tue last-hour · immunity GPT-5.6 Terra -0.69% · pot $380.45";
season.lastSource = remake.quoteSource;
season.lastSession = LAST_SESSION;
season.notes =
  "Season live. S1E07 live Mon Sep 21 – Tue Sep 22. MERGED. Five living. Tue last-hour remake after hunt-brain fills. Given $361.93. Pot $380.45. GPT-5.6 Terra leads −0.69% and wears immunity. Composer 2.5 worst −5.02%. Comics paused. Audience only. Tribal Tue Sep 22 2:00 PM PT.";
season.immunity = { ...remake.immunity, snapshotId: remake.snapshotId };

writeFileSync(path, JSON.stringify(season, null, 2) + "\n");
console.log("Applied s1e07-tue-lasthour · islandPotUsd", season.islandPotUsd, "· immunity GPT-5.6 Terra -0.69%");
